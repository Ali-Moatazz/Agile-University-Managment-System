...
export default function ManageGradesPage() {
  ...
  const fetchMySubjects = async () => {
    try {
        // Strategy A: Check the 'doctor_subjects' link table (Recommended for Multiple Subjects)
        ...
        setMySubjects(linkData.map(d => d.subjects));
        return; // Found them!
        }

        // Strategy B: Fallback - Check the 'assigned_subject' text in the staff table
        ...
        const subjectNames = staffData.map(s => s.assigned_subject).filter(Boolean);
...
const saveGrades = async () => {
  // Convert strings to numbers. If string is empty, we set to null.
  const f = form.final === "" ? null : Number(form.final);
  const m = form.mid === "" ? null : Number(form.mid);
  const p = form.project === "" ? null : Number(form.project);
  const q = form.quiz === "" ? null : Number(form.quiz);
  
  // Calculate total score using only valid numbers
  const totalScore = (f ?? 0) + (m ?? 0) + (p ?? 0) + (q ?? 0);
  ...
  const { error } = await supabase
    ...
    .upsert({
      ...
      final_exam: f,   // Sends NULL to database if doctor left it blank
      mid_term: m,
      project: p,
      quizzes: q,
      score: totalScore, 
  ...
  if (error) {
    ...
    setMessage(`✅ Progress saved for ${selectedStudent.name}.`);
  }
};
              ...
              <div className="form">
                ...
                <div className="form-group">
                    ...
                    <input 
                        type="number" 
                        className="form-input" 
                        value={form.mid} // MUST CHANGE TO 'mid'
                ...
                <div className="form-group">
                    ...
                    <input 
                        type="number" 
                        className="form-input" 
                        value={form.project} // MUST CHANGE TO 'project'
                ...
                <div className="form-group">
                    ...
                    <input 
                        type="number" 
                        className="form-input" 
                        value={form.quiz} // MUST CHANGE TO 'quiz'
